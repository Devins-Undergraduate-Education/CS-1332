...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        // If no duplicates found, add a new entry to the front of the chain
    ...
    public V remove(K key) {
        ...
        throw new java.util.NoSuchElementException("Key not found in map");
    }
...
public Set<K> keySet() {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("New length is less than the number of items in the hash map.");
        }
    
        // Create new array with the specified length
        ExternalChainingMapEntry<K, V>[] newTable = new ExternalChainingMapEntry[length];
    
        // Iterate over the old array and rehash all entries into the new array
        for (ExternalChainingMapEntry<K, V> entry : table) {
            while (entry != null) {
                K key = entry.getKey();
                ...
                if (newTable[index] == null) {
                    newTable[index] = new ExternalChainingMapEntry<>(key, value);
                } else {
                    ExternalChainingMapEntry<K, V> prev = null;
                    ExternalChainingMapEntry<K, V> current = newTable[index];
                    ...
                    prev.setNext(new ExternalChainingMapEntry<>(key, value));
                }
